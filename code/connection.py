...
def Connect():
    ...
    # get handles
    status, joint1_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint1',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
        raise Exception("Cannot get handle of first joint")

    status, joint2_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint2',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
        raise Exception("Cannot get handle of second joint")

    status, joint3_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint3',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
        raise Exception("Cannot get handle of third joint")

    status, joint4_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint4',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
        raise Exception("Cannot get handle of fourth joint")

    status, joint5_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint5',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
        raise Exception("Cannot get handle of fifth joint")

    status, joint6_handle = vrep.simxGetObjectHandle(clientID, 'Jaco_joint6',vrep.simx_opmode_blocking)
    if status != vrep.simx_return_ok:
